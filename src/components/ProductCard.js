...
export default function ProductCard(prop) {
  ...
  const { user, setDetectChange } = useContext(UserContext);
    ...
    <Fragment>
      <Col xs={12} md={4} className="p-3 mb-4 filter-target">
        <Card className="cardHighlight border-dark">
          ...
          <Card.Body className="">
            <Card.Title className="text-center title-card" onClick={handleShow}>
              <h3>{name}</h3>
            </Card.Title>
      ...
      <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Footer>
          {user.id !== null ? (
            user.isAdmin ? (
              <Button disabled variant="outline-warning" onClick={addToCart}>
              ...
              <Button variant="outline-warning" onClick={addToCart}>
                <Badge className="bg-danger" variant="danger">
                  {count}
                </Badge>{" "}
                Add to Cart!
