...
export default function Cart(prop) {
  ...
  return (
    <div className="d-flex align-items-center">
      <Button className="rounded-pill" onClick={handleShow} variant="warning">
        Your Cart{" "}
        <Badge className="bg-danger ml-2" variant="danger">
          {prop.tempArray.length}
        </Badge>
      </Button>
      <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Footer>
          {isActive ? (
            <Button className="mr-auto" variant="success" onClick={checkout}>
              Checkout
            </Button>
          ) : (
            <Button className="mr-auto" variant="success" disabled>
              Checkout
            </Button>
          )}
