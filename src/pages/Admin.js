...
import { Form, Button, ListGroup, Dropdown, Modal, Row } from "react-bootstrap";
...
export default function Admin() {
  const { user, detectChange, setDetectChange, filterInput, api } =
      ...
      .then((data) => {
        setProducts(
          data
            .filter((datum) => {
              return (
                datum.name.toUpperCase().indexOf(filterInput.toUpperCase()) >= 0
              );
            })
            .map((product) => {
              return <ProductDatum key={product._id} datum={product} />;
            })
        );
  ...
  return (
    <Form className="mt-3 pt-5 mt-5">
      <Form.Group>
        <div id="admin-header">
          <div
            id="admin-header-content"
            className="mt-3 text-center rounded-pill p-3"
          >
      ...
      <Form.Group className="mt-5 pt-5 pb-3 d-md-flex justify-content-around border-bottom border-dark">
        ...
        <ListGroup defaultActiveKey="#link2" className="d-none d-md-block">
          ...
          <ListGroup.Item
            onClick={() => {
              alert(`Feature to be contructed after satisfied CSS`);
            }}
            className="rounded mb-3"
            action
            href="#link5"
      ...
      {route === "viewAll" ? (
        <Form.Group className="mt-5">
          <Filter />
          <Row className="d-flex justify-content-center">{products}</Row>
        </Form.Group>
      ) : route === "orders" ? (
        <Form.Group>
          <Filter />
          <Row>{orderTickets}</Row>
        </Form.Group>
      ) : (
